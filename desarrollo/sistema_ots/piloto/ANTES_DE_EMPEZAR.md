...
✅ **Hecho el 2026-09-13 (noche), a pedido explícito de Andrés.** Las baterías dejaban cuentas de prueba (`tec_prueba_uio_a`, `tec_prueba_uio_b`, `jefe_prueba_uio`, `jefe_prueba_cnlj`, `admin_prueba`), dos órdenes reales de UIO asignadas a una cuenta de prueba, avisos sintéticos `9999xxxx`, cuatro solicitudes de repuesto y dos novedades marcadas PRUEBA, y 72 OT INDUSTEC de la serie 9000. Se revirtió con un solo comando, por SSH, desde la carpeta `ot/` del sitio de pruebas:
...
La app exige que quien firma una OT INDUSTEC esté en el padrón (`catalogos/tecnicos.json`) y que el local y los equipos estén en el maestro. Los publica la estación.
...
En el servidor, el 13 de septiembre de 2026: **159 diagnósticos pre-redactados**, **109 repuestos frecuentes**, **368 ingresos del cronograma preventivo** y **209 OT INDUSTEC en el Archivo** (180 con PDF en Hostinger). El histórico completo de 7.069 OT INDUSTEC entra al Archivo cuando Andrés decida subir los PDF (D2 del PLAN); hasta entonces el índice muestra «pedir copia» en las que solo están en la estación.

## 6. El buzón de SAP sigue llegando al sistema nuevo

Las órdenes nuevas entran al `Buzón de órdenes` porque la estación vigila el correo de SAP y las empuja al sitio de pruebas (`t2_9_buzon_vigilante.py`).
...
- Se agrega a la pantalla de inicio (Chrome/Edge en Android: menú → «Agregar a la pantalla principal»; Safari en iPhone: compartir → «Agregar a inicio»). Así abre como una app y guarda las OT INDUSTEC sin señal.
- Quien ya la tenía instalada de antes recibe sola la versión nueva (`sw.js` v19 o posterior, que precarga también `vocabulario.json`): basta con cerrarla y volver a abrirla.

**Comprobación:** con el celular en modo avión, la app abre, muestra Mis órdenes (con la barra «Mis órdenes · Historial · Emitir · Repuestos · Preventivos») y deja llenar una OT INDUSTEC; al volver la señal, la OT INDUSTEC sale sola y aparece su número.
...
## 9b. Las capturas de las hojas

Las de `capturas/` son del 13 de septiembre de 2026 y muestran los rótulos anteriores al vocabulario único. Antes de entregar el paquete, volver a tomarlas contra el sitio de pruebas (ver [`LEEME.md`](LEEME.md)), ya con los términos nuevos desplegados.

**Comprobación:** en `capturas/admin/panel_php.png` se ve la tarjeta «Por zona» con las filas ÓRDENES ABIERTAS · ÓRDENES A ESPERA DE INFORME TÉCNICO · EQUIPOS DESHABILITADOS · TOTAL DE ÓRDENES ABIERTAS, y en `capturas/tecnico/mis_php.png` las pestañas Asignadas · A espera de repuesto · Historial · Notificaciones.
...
| 3 | Decisión sobre las OT INDUSTEC 90xx | anotada en ESTADO.md | ✅ 2026-09-13 — se borraron |
