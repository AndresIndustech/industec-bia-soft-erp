...
| `tarjeta_cli.php` | servidor, desde `ot/` | La tarjeta «Por zona» del panel calculada contra la base REAL con las mismas funciones de `panel.php` (`ADMIN` o `JEFE_ZONA:<zona>`): ejecuta las consultas SQL nuevas de `Casos::informesPorAviso()` que ninguna otra prueba ve y comprueba sus invariantes (TOTAL = ABIERTAS + A ESPERA; las zonas suman el total; deshabilitados y vencidas son subconjuntos; TOTAL ≤ conteo directo por estado). **Solo imprime agregados**, nunca avisos ni personas | No |
