# Notas para César: la nueva página web de INDUSTEC

> **De**: Valentina (comercial y marketing de INDUSTECH), a través de Andrés
> **Para**: César Basantes, INDUSTEC
> **Fecha**: 11 de septiembre de 2026

César, la nueva página usa **solo lo que hoy ya dice www.industec.me**, mejor ordenado y más fácil de leer en el celular. No agregamos ningún dato de su operación.

**Lo que cambiamos el 11 de septiembre de 2026, por lo que usted le pidió a Andrés**: la página ahora deja claro que INDUSTEC es un proveedor técnico de **empresas y cadenas** (cadenas de comida rápida, restaurantes, hoteles, bares y cafeterías, catering), que trabaja con contrato de servicio y que **no repara equipos domésticos ni atiende a particulares**. La portada muestra un dibujo propio de la cocina de un local de cadena, con freidoras, mantenedor de calor, refrigeración y campana de extracción, no una cocina de casa, y la página habla de **línea caliente, línea fría y ventilación**, no solo de freidoras. También muestra los **logos** de sus clientes y de las marcas de equipos. Solo necesitamos su confirmación en dos puntos: la **A6** (los logos) y la **B15** (a quién atienden), y algunos detalles del dibujo de la portada (ver B2).

Aquí está lo que le propongo **corregir o agregar**. **Nada de esto se publica hasta que usted diga que sí.** Basta con marcar Sí o No en la tabla de abajo y, donde haga falta, mandarnos el dato o la foto.

---

## Resumen para decidir rápido

| # | Propuesta | Qué necesitamos de usted | ¿Sí o No? |
|---|---|---|---|
| A1–A2 | Quitar las dos citas de estudios internacionales | Solo su aprobación (ya las quitamos del borrador) | [ ] Sí · [ ] No |
| A3 | Quitar el «previene hasta un 85% de fallas» o respaldarlo con datos suyos | ¿Tiene registros propios? | [ ] Quitar · [ ] Tengo datos |
| A5 | Tratar al cliente de «tú» en toda la web | Su preferencia | [ ] Tú · [ ] Usted |
| A6 | Revisar los nombres de clientes **y autorizar que se muestren sus logos** | Confirmar la escritura correcta, que está de acuerdo con que se muestren los logos | [ ] Revisado |
| **B1** | **Decir en qué ciudad están y a dónde van** | Ciudad y zona de cobertura | [ ] Sí · [ ] No |
| **B2** | **Fotos reales de su trabajo** (la página ya usa 5 fotos de su web actual, más un detalle del dibujo de la portada que confirmar) | Confirmar que son suyas, el archivo original de la foto de las freidoras y 1 o 2 fotos de refrigeración o de una campana de extracción (no hay ninguna todavía) | [ ] Sí · [ ] No |
| **B3** | **Horario de oficina y confirmar el 24/7** | Horario y número de emergencias | [ ] Sí · [ ] No |
| **B4** | **Nombrar los demás tipos de equipos que atienden** (la página ya habla de línea caliente, línea fría y ventilación en general) | Marcar la lista de equipos concretos | [ ] Sí · [ ] No |
| B5 | Testimonios de clientes | 2 o 3 clientes que acepten, por escrito | [ ] Sí · [ ] No |
| B6 | Casos de trabajo cortos | 2 o 3 historias | [ ] Sí · [ ] No |
| B7 | Certificaciones o autorizaciones de marcas | Documento que lo respalde | [ ] Sí · [ ] No |
| B8 | Explicar qué incluye la consultoría gratuita | Qué incluye | [ ] Sí · [ ] No |
| B9 | Cifras que usted quiera mostrar | Usted decide cuáles | [ ] Sí · [ ] No |
| B10 | Contar que el cliente recibe reportes digitales | Que lo decida con Andrés | [ ] Sí · [ ] No |
| B11 | Redes sociales y ficha en Google Maps | Enlaces, o crear la ficha | [ ] Sí · [ ] No |
| B12 | Dirección y mapa | Solo si reciben visitas | [ ] Sí · [ ] No |
| B13 | Volver a poner «Trabaja con nosotros» (la retiramos del borrador) | A qué correo llegan las hojas de vida | [ ] Sí · [ ] No |
| B14 | Aviso de privacidad | Razón social y dirección | [ ] Sí · [ ] No |
| **B15** | **Confirmar a quién atienden** (la web ya dice «bajo contrato de servicio») | ¿Atienden también, de forma puntual, a una empresa que aún no tiene contrato? | [ ] Solo con contrato · [ ] También sin contrato |

---

## A. Lo que corregimos de la web actual

### A1. Cita de The Business Research Company (sobre el crecimiento del sector)

La web dice: «crecimiento anual del 5,6%… para 2028 el mercado global del sector hotelero alcanzará los 5,92 billones de dólares».

- **No encontramos esas cifras** en las ediciones del informe que consultamos el 11 de septiembre de 2026.
- La edición 2026 del informe (*Hospitality Global Market Report*) dice otra cosa: USD 5,52 billones en 2025 y USD 7,48 billones en 2030, un 6,4 % de crecimiento anual. Además, esas cifras son de **toda la hospitalidad** (alojamiento, restaurantes, cafeterías, catering y bares), no solo del «sector hotelero».
- **Propuesta**: quitarla. Al dueño de un restaurante en Ecuador el tamaño del mercado mundial no le ayuda a decidir. Si usted prefiere conservar una cifra, usaríamos la actualizada y con su fuente.

### A2. Cita de Oracle («el 83% de los hoteles…»)

- **No encontramos esa cifra** en las publicaciones de Oracle Hospitality que revisamos. Además, la web la atribuye a un informe de *restaurantes* cuando habla de *hoteles*.
- **Propuesta**: quitarla. Si usted tiene el documento original, la reponemos con su enlace.

### A3. «Previene hasta un 85% de fallas inesperadas»

- No encontramos de dónde sale. Las fuentes que hallamos hablan de mucho menos, y además son de empresas que venden ese servicio, así que tampoco sirven de respaldo.
- Una cifra así, si un cliente la cuestiona, **resta confianza** y puede dar pie a un reclamo por publicidad engañosa.
- **Propuesta**: en el borrador ya dice «para que tus equipos no fallen», sin porcentaje. **Si usted tiene datos propios** (por ejemplo, cuántas fallas tenía un local antes y después del plan preventivo), armamos la cifra con ese respaldo.

### A4. Errores que ya corregimos

- En la web actual los títulos de dos listas están **cruzados**: los fabricantes (Hobart, Frymaster…) salen como «Nuestros clientes» y los restaurantes (KFC, Cinnabon…) como «Marcas con las que trabajamos».
- Errores de tipeo: «sócio», «negócio», «contatará», «Respuestos», «correccion».

### A5. ¿Tú o usted?

La web actual mezcla las dos formas: «tu restaurante» en la portada, «Describa su necesidad» en el contacto. Unificamos en **tú**, que es más cercano y es lo que más se usaba. El logo dice «La solución a **sus** equipos», y el logo no se toca. Si usted prefiere «usted», se cambia en toda la web.

### A6. Nombres de clientes: confirmar la escritura

La nueva página muestra los **20 clientes** de la galería de su web actual, con cada nombre escrito como lo dice su propio logo. Por favor confirme:

- **Vaco y Vaca**, tal como dice su logo (en la web actual el archivo se llama «V&V»): confirmar.
- **Decameron**, **Naturíssimo**, **TropiBurger** y **Corporación GPF**: los escribimos como en su logo.
- **American Deli**, **Gus** y **TEVCOL**: confirmar la escritura.
- **Ali's Parrilladas & Pizzería**, **El Español** e **Il Cappo di Mangi**: están en su web actual, pero se nos habían quedado fuera porque sus archivos no dicen de qué empresa son. Ya están en la página nueva: confirme que siguen siendo sus clientes y cómo se escriben.

Confirme también que está de acuerdo con seguir mostrando a sus clientes, que hoy ya aparecen en la web. **Importante**: los logos de clientes y marcas y las fotos que usa la página nueva ya los respaldamos. Del resto de las fotos de «Nuestro trabajo» (24 de las 33) no hay copia: si quiere conservarlas, hay que **descargarlas antes de cerrar la web de Zyro**; si no, se pierden.

**Logos (11 de septiembre de 2026)**: por pedido de Andrés, la nueva página muestra los **logos** de sus clientes y de las marcas de equipos, no solo los nombres, como hace la web actual. Debajo de los logos de clientes dice: «Los logos pertenecen a cada empresa y se muestran como referencia de trabajos realizados.» Debajo de las marcas: «Las marcas y sus logos pertenecen a sus fabricantes. Mencionarlas no significa que INDUSTEC sea servicio técnico autorizado ni distribuidor oficial de ellas.» Si algún cliente le ha pedido no aparecer, díganos cuál y lo quitamos.

Algunos logos tienen un detalle:

- **San Felipe** solo existe en su web en color blanco, así que lo mostramos en gris oscuro para que se vea sobre fondo claro.
- **Arrayanes**, **Decameron** e **Il Cappo di Mangi** son archivos pequeños y se ven algo borrosos en pantallas de alta resolución. Al de Il Cappo di Mangi le quitamos la franja con la bandera y el fondo beige, para que quede como los demás.
- **Menestras del Negro**: le quitamos el lema «Como preparado en casa.». Es la única mención a «casa» de toda la página y está justo en la sección que muestra que trabajan con empresas. ¿Tiene una versión oficial del logo sin el lema? Si prefiere mostrarlo completo, lo reponemos.

Si tiene mejores versiones de cualquiera de estos logos, las cambiamos.

**Marcas de equipos**: la lista abre con los fabricantes de cocina profesional (Frymaster, Henny Penny, Dean, Hobart…), sigue con refrigeración y extracción, y cierra con **Samsung, LG y Westinghouse**, que están en su web actual. Andrés pidió mantener las 18 para que la lista quede completa. **SilverChef**, que aparece entre las marcas de su web actual, no pasa a la página nueva: es una empresa que financia equipos, no un fabricante.

---

## B. Lo que propongo agregar (necesita su visto bueno)

> Todo lo de esta sección está marcado **[CONFIRMAR CON CÉSAR]** y **no está en la página** hasta que usted lo apruebe.

### Prioridad alta: esto es lo que más ayuda a conseguir clientes

**B1. Ciudad y cobertura [CONFIRMAR CON CÉSAR]**
Hoy la web **no dice dónde están**. Quien busca en Google «mantenimiento de equipos de restaurante en Quito» (o en su ciudad) no los encuentra.
- *Qué necesito*: la ciudad base y las ciudades o provincias que atienden. **Sin decir cuántos locales ni cómo se organizan internamente.**
- *Dónde iría*: portada, contacto y datos para Google.

**B2. Fotos reales de su trabajo [CONFIRMAR CON CÉSAR]**
Desde el 11 de septiembre de 2026 la página usa **cinco fotos que hoy ya están en su web actual**:
- en la sección «Nuestro trabajo» de la página de inicio y de servicios, y en la de contacto: el técnico con casco y chaleco revisando freidoras (la misma foto de la portada de su web actual, con un tratamiento de color más oscuro y de más contraste), una vitrina caliente con el tablero eléctrico abierto, dos técnicos reparando un horno y los mantenedores de calor de una línea de despacho.

Las recortamos para que no se vean caras ni letreros o logos de clientes.
- *Qué necesito*:
  1. **Que confirme que esas fotos son suyas** y que ningún cliente se opone a que se vean sus cocinas.
  2. **Que confirme que el técnico de las freidoras es de su personal** (no se le ve la cara). Su descripción para lectores de pantalla dice «Técnico de INDUSTEC revisando freidoras…».
  3. **El archivo original de esa foto** (IMG_6213, tal como salió del teléfono). La copia que guarda su web actual mide solo 712 píxeles de ancho: se ve algo blanda y el casco queda cortado arriba.
  4. **Que confirme que el tratamiento de color más oscuro (la foto se ve más de noche que de día) se puede publicar así.**
  5. **Una o dos fotos de refrigeración (una cámara fría o un refrigerador comercial) o de una campana de extracción**, en la cocina de una cadena y **sin logos del cliente a la vista**. Hoy ninguna foto real muestra línea fría ni ventilación: la portada la representa con un dibujo, no con una foto.

La portada ya no lleva esta foto: desde el 11 de septiembre de 2026 (tarde) es un **dibujo propio** de la cocina de un local de cadena, en el mismo estilo del primer borrador que usted vio, pero con freidoras, mantenedor de calor, refrigeración y campana en vez de una cocina de casa. Cuatro detalles del dibujo que también necesitamos que confirme:
  6. **Los colores del uniforme** del técnico dibujado: casco blanco, overol azul y chaleco naranja reflectivo. ¿Así viste su personal, o prefiere otros colores?
  7. La pantalla del refrigerador dibujado marca **«3°»**. Es solo un ejemplo, no un dato suyo: dígannos si lo quitamos.
  8. En la esquina superior izquierda hay una **insignia redonda con una llave**. Si prefiere el dibujo sin ella, ya tenemos lista esa versión.
  9. El dibujo no incluye una **plancha** ni una **máquina de hielo**, para que el técnico y las freidoras no se vean pequeños. Si prefiere que aparezcan, se puede cambiar la mesa de refrigeración o el mantenedor de calor por una de ellas.

Las fotos reales generan más confianza que cualquier texto. Otras sugerencias para completar la galería:
1. Un técnico trabajando en un equipo (de cerca: manos y herramientas).
2. Un antes y después de una limpieza profunda.
3. Un técnico con el uniforme de INDUSTEC.
4. Un diagnóstico o una calibración (instrumento en mano).
5. Una capacitación al personal de un local.
6. Un equipo recién instalado y funcionando.
7. El vehículo o las herramientas de la empresa.
8. Usted, como fundador, para la página «Nosotros» (opcional).

*Cuidados*: permiso de las personas que salen en la foto, y que no se vea el logo ni el local de un cliente sin su autorización. Fotos horizontales, con buena luz; con el celular basta.

**B3. Horario y emergencias 24/7 [CONFIRMAR CON CÉSAR]**
- La web promete «servicio de emergencia 24/7». ¿Sigue vigente? ¿Se atiende en el mismo 099 788 7709?
- ¿Cuál es el horario de oficina, para consultas que no son urgentes?
- La página de contacto dice ahora «Te atiende nuestro equipo». ¿El WhatsApp, la llamada y el correo los responde su equipo técnico, o otra persona (por ejemplo, recepción o comercial)? Si es el equipo técnico, lo decimos así: «Te atiende nuestro equipo técnico».
- Esa misma página describe en tres pasos, sin horas ni plazos, cómo se atiende una consulta: 1) nos cuenta qué pasa, 2) conversamos y revisamos el caso, 3) coordinamos la solución (mantenimiento preventivo, predictivo o correctivo, bajo contrato). ¿Así es como trabajan?

**B4. Tipos de equipos que atienden [CONFIRMAR CON CÉSAR]**
Hoy solo se deduce de las marcas y de un dibujo. Nombrarlos ayuda en Google y deja claro qué hacen. **La página ya habla de «línea caliente, línea fría y ventilación»** en general (pedido de Andrés el 11 de septiembre de 2026), pero sin decir cada equipo concreto. Marque los que correspondan:
[ ] Cocción (cocinas, hornos, planchas) · [ ] Refrigeración y cámaras de frío · [ ] Máquinas de hielo · [ ] Bebidas, café y helados · [ ] Campanas y extracción · [ ] Lavado de vajilla · [ ] Otros: ______

### Prioridad media: dan confianza

**B5. Testimonios [CONFIRMAR CON CÉSAR]**
Dos o tres frases de clientes satisfechos, con nombre, cargo y empresa, y **su autorización por escrito** (un correo basta).

**B6. Casos de trabajo [CONFIRMAR CON CÉSAR]**
Dos o tres historias cortas: problema → qué hicimos → resultado. Sin datos del contrato ni cifras internas, salvo las que usted apruebe.

**B7. Certificaciones y autorizaciones [CONFIRMAR CON CÉSAR]**
La web actual hablaba de «estándares de certificación internacional», una afirmación que no pudimos respaldar (ver A1–A3): ya la quitamos, en Nosotros y en servicios. ¿INDUSTEC es servicio técnico autorizado de alguna de las marcas (Hobart, Frymaster, Henny Penny…)? Si es así, es un gran diferenciador, pero **solo se publica con el documento que lo respalde**. Hoy la web no lo afirma, y nosotros tampoco.

**B8. Consultoría integral gratuita [CONFIRMAR CON CÉSAR]**
Es el botón principal de la web actual y lo mantenemos. ¿Sigue vigente? ¿Qué incluye (visita, diagnóstico, informe)? Explicarlo en una línea hace que más gente la pida.

**B9. Cifras que usted quiera mostrar [CONFIRMAR CON CÉSAR]**
Los números bien elegidos convencen. Usted decide cuáles se muestran. Ideas: años de experiencia del fundador (el «legado de más de 40 años»: ¿es su trayectoria personal?), tiempo promedio de respuesta en emergencias (solo si se mide y se cumple).
**Lo que no publicamos** (regla nuestra): número de locales atendidos, órdenes de trabajo por mes, zonas, nombres del personal, RUC ni datos de contratos.

**B10. Reportes digitales para sus clientes [CONFIRMAR CON CÉSAR y ANDRÉS]**
La web actual promete «tecnología de vanguardia», pero no la explica. Si con el nuevo sistema B.IA Soft ERP sus clientes van a recibir un **reporte digital de cada servicio** (con fotos y trabajo realizado), eso es un diferenciador fuerte para contar en la web. Solo lo publicamos si ya se está entregando o si usted decide ofrecerlo.

### Prioridad baja: completan la presencia

**B11. Redes sociales y Google Maps [CONFIRMAR CON CÉSAR]**
- ¿Tienen Facebook, Instagram o LinkedIn? Si existen, se enlazan en el pie de página.
- Recomiendo crear el **Perfil de Empresa en Google** (la ficha que aparece en Google Maps). Es gratis y es donde más buscan los dueños de locales. Pide dirección o zona de servicio (ver B1).

**B12. Dirección y mapa [CONFIRMAR CON CÉSAR]**
Solo si reciben clientes en una oficina o taller. Si no, basta con la zona de cobertura (B1).

**B13. «Trabaja con nosotros» [CONFIRMAR CON CÉSAR]**
Según nuestra revisión, la web actual tiene esta sección. En la nueva página **la retiramos hasta que usted confirme**, por dos motivos: no sabemos a qué correo deben llegar las hojas de vida, y las hojas de vida son datos personales que hay que guardar con cuidado y borrar cuando ya no se necesiten (ver B14).
- *Qué necesito*: ¿la volvemos a poner? Si es así, ¿a qué correo deben llegar las hojas de vida? Si no nos dice otro, llegarían a servicioalcliente@industec.me.
- *Texto listo para reponerla*: «¿Quieres formar parte de nuestro equipo? Siempre buscamos talento apasionado por la innovación.», con el botón «Escríbenos».

**B14. Aviso de privacidad [CONFIRMAR CON CÉSAR]**
El formulario de la nueva web **no guarda nada**: solo arma el mensaje para WhatsApp o correo. Aun así, al recibir esos mensajes INDUSTEC trata datos personales, y la Ley Orgánica de Protección de Datos Personales pide informar para qué se usan. Propongo un aviso corto junto al formulario y una página de privacidad.

*Texto modelo* (versión del 11 de septiembre de 2026, **como referencia**; usted lo valida, e idealmente lo revisa un abogado):

> «Cuando nos escribes por WhatsApp, correo o teléfono, INDUSTEC usa tu nombre, tus datos de contacto y lo que nos cuentas únicamente para responder tu solicitud y darte el servicio. No los vendemos ni los compartimos con terceros con fines publicitarios. Puedes pedir que los corrijamos o eliminemos escribiendo a servicioalcliente@industec.me. Responsable: [RAZÓN SOCIAL], [DIRECCIÓN].»

*Qué necesito*: razón social y dirección para completarlo.

### Nuevo (11 de septiembre de 2026): confirme un matiz de lo que ya está en la página

**B15. A quién atienden [CONFIRMAR CON CÉSAR]**
Siguiendo lo que usted le dijo a Andrés, la página dice ahora, en la portada y en contacto:

> «Atendemos a empresas y cadenas, bajo contrato de servicio. No reparamos equipos domésticos ni atendemos a particulares.»

Y el formulario pide, como dato obligatorio, el nombre de la **empresa o cadena**.
- *Qué necesito*: ¿atienden también, de forma puntual, a una **empresa sin contrato** (por ejemplo, un hotel o una cafetería que les escribe por primera vez con una emergencia)? ¿El servicio de emergencia 24/7 es solo para clientes con contrato? (se une a la B3)
- *Si la respuesta es sí*, cambiamos la línea por «Atendemos a empresas y cadenas del sector. No reparamos equipos domésticos ni atendemos a particulares.», para no cerrarle la puerta a una empresa que llega por primera vez.
- *Si es no*, la dejamos como está.

---

## C. Lo que ya incluye la nueva página (para que lo tenga presente)

- **Portada clara**: en la primera pantalla dice qué hacen, para quién y cómo contactarlos, con los botones WhatsApp y Llamar.
- **Enfoque en empresas y cadenas** (11 de septiembre de 2026): la sección «A quién servimos» (cadenas de comida rápida, restaurantes, hoteles, bares y cafeterías, catering), la línea que aclara que no reparan equipos domésticos, y el campo «Empresa o cadena» del formulario, que llega en cada mensaje.
- **Portada con un dibujo propio de la cocina de un local de cadena** (tarde del 11 de septiembre de 2026), con freidoras, mantenedor de calor, refrigeración y campana de extracción, en el estilo del primer borrador pero sin nada que se lea como cocina de casa. Debajo del dibujo van tres distintivos: «Línea caliente, línea fría y ventilación», «Mantenimiento preventivo, predictivo y correctivo» y «Servicio para cadenas de restaurantes». Los logos de sus clientes y de las marcas quedan justo después de «A quién servimos», y la sección «Nuestro trabajo» muestra cuatro fotos de trabajos, entre ellas la foto real del técnico con las freidoras (ver B2).
- **Imagen para compartir en WhatsApp y redes** con el mismo dibujo, el logo y el titular «Mantenimiento de cocinas profesionales para cadenas de restaurantes y hoteles».
- **WhatsApp a un toque** en todas las páginas, con el mensaje ya escrito según el servicio.
- **Guía «¿Qué servicio necesitas?»**: el cliente elige su situación («un equipo ya falló», «la falla se repite»…) y llega al servicio correcto.
- **Acceso del personal**: página `/acceso/` con los botones «Ingresar al sistema» y «Abrir la app del técnico». No aparece en Google.
- **Sin rastreadores ni publicidad** en la página.

---

## D. Cómo sabremos si la página funciona (sin rastreadores)

- Todo mensaje de WhatsApp enviado desde la web empieza con **«les escribo desde su página web»**. Basta con contar cuántos llegan cada semana.
- Los correos llegan con el asunto **«Solicitud desde la web»**.
- En las llamadas, preguntar: «¿Cómo nos encontró?».
- **Meta**: [DEFINIR CON CÉSAR]. Propongo tomar el **primer mes en el dominio definitivo como línea base** y fijar la meta con ese número real, no con uno inventado.

---

## E. Para Andrés (técnico): el día que se pase a www.industec.me

1. Quitar `<meta name="robots" content="noindex, nofollow">` de `/`, `/nosotros/`, `/servicios/` y `/contacto/`. **En `/acceso/` se queda.**
2. Cambiar `robots.txt` a:
   ```
   User-agent: *
   Disallow: /ot/
   Sitemap: https://www.industec.me/sitemap.xml
   ```
   `/acceso/` no se bloquea en `robots.txt`: si se bloquea, Google no llega a leer su `noindex`.
3. Añadir `<link rel="canonical">` en cada página y un `sitemap.xml` con las 4 páginas públicas (sin `/acceso/` ni `/ot/`).
4. Cambiar al dominio definitivo las URL absolutas de `og:url`, `og:image` y los datos estructurados (`contenido.md`, sección 9.3).
5. Comprobar que las direcciones viejas de Zyro sigan funcionando: `/nosotros` debe llevar a `/nosotros/` (Apache lo hace solo, sin `.htaccess`).
6. **Correo**: al mover el dominio desde Zyro, **conservar los registros MX, SPF y DKIM** de `industec.me` para que servicioalcliente@industec.me no deje de recibir correos.
7. **Antes de dar de baja Zyro**: descargar el resto de las fotos de «Nuestro trabajo» (24 de 33), si se quieren conservar. Los logos y las fotos que usa la página nueva ya están respaldados (ver A6).
8. Registrar el sitio en Google Search Console a nombre de César (se verifica por DNS; no añade scripts a la página) y, si se aprueba B11, crear el Perfil de Empresa en Google.

---

## Fuentes consultadas el 11 de septiembre de 2026

- The Business Research Company, *Hospitality Global Market Report 2026*: https://www.thebusinessresearchcompany.com/report/hospitality-global-market-report (USD 5.523,92 miles de millones en 2025; USD 7.475,3 miles de millones en 2030; crecimiento anual compuesto de 6,4 %).
- *Hospitality Global Market Report 2025* (TBRC, distribuido por GII Research): https://www.giiresearch.com/report/tbrc1804941-hospitality-global-market-report.html (USD 5.244,22 miles de millones en 2024 y USD 5.523,92 miles de millones en 2025, con un crecimiento del 5,3 %; no aparecen el 5,6 % ni los 5,92 billones para 2028).
- Oracle Hospitality, estudio *Hospitality in 2025* (junio de 2022): https://www.oracle.com/news/announcement/oracle-hospitality-in-2025-consumer-research-study-2022-06-01/ y el estudio Oracle-Skift *Hospitality in 2025: automated, intelligent… and more personal*: https://www.oracle.com/a/ocom/docs/industries/hospitality/hospitality-industry-trends-for-2025.pdf. En los resultados revisados no aparece la cifra del 83 %.
- Para el 85 %: búsquedas sobre mantenimiento preventivo en cocinas comerciales; la única cifra hallada (30 a 40 % menos averías) es de un proveedor de software de mantenimiento (https://oxmaint.com/industries/hospitality/commercial-kitchen-equipment-maintenance-checklist-hospitality), no de un estudio independiente.
