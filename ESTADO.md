...
| **T2.19 · Todos los PDF en el servidor** (2026-09-14) | ⏸ escrito y probado desde el PC; **falta correrlo en la estación** (`D:\RESPALDOS` no existe en el PC) | `t2_19_subir_pdfs.py`: **16/16** pruebas sin red (`t2_19_pruebas.py`). De punta a punta contra el servidor, en `~/respaldos/prueba_t2_19` con un corpus sintético: 7 subidos en 4 lotes verificados en el destino, segunda corrida 0, un divergente reportado y no pisado, huellas del servidor = huellas locales, 0 temporales sobrantes; simulación contra el `ordenes_pdf` real (solo lectura) sin error. Paso `pdfs` al final de `saneamiento_nocturno.py` |
