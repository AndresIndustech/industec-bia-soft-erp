...
|---|---|---|---|
| **T2.21 · producción como fuente de informes** — cambios SIN COMMITEAR en el disco, sin verificar | Cortada el 2026-09-20 por límite de sesión (`resets 2:50pm`) | 2026-09-19/20 | `git status` en la estación muestra `comun.py`, `informes.bat`, `t2_11_informes_ot.py`, `t2_4_sync_hostinger.py` y `t2_9_buzon_vigilante.py` modificados (229 líneas +, 76 −) por el agente programador, que murió por el límite ANTES de commitear. **No confíes en el disco sin leer el diff primero** (mismo error nº 15 del plan): puede compilar y aun así no hacer lo que dice el encargo. Ver §11b para el encargo completo y qué falta verificar |
